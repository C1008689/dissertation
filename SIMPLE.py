...
def plot_neural_network_contextual():
    ...
    for i, layer in enumerate(layers):
        ...
        for (x, y) in pos:
            ...
            ax.add_patch(circle)
            # Remove node annotations for clarity

    # Plot connections with different styles for better visibility
    for i in range(len(layers) - 1):
        for (x1, y1) in positions[i]:
            for (x2, y2) in positions[i + 1]:
                line = plt.Line2D([x1, x2], [y1, y2], color='k', alpha=0.3, linestyle='--' if i % 2 == 0 else '-', linewidth=0.5)
                ax.add_line(line)

    # Annotations with increased font size
    plt.text(-0.75, input_layer / 2 - 0.5, 'Input Layer\n(Book Reviews)', fontsize=16, ha='center', va='center', bbox=dict(facecolor='white', alpha=0.8))
    for i in range(1, len(hidden_layers) + 1):
        plt.text(i * 1.5, hidden_layers[i-1] / 2 - 0.5, f'Hidden Layer {i}\n(Feature Extraction)', fontsize=16, ha='center', va='center', bbox=dict(facecolor='white', alpha=0.8))
    plt.text(len(hidden_layers) * 1.5 + 1.5, output_layer / 2 - 0.5, 'Output Layer\n(Sentiment Classification)', fontsize=16, ha='center', va='center', bbox=dict(facecolor='white', alpha=0.8))

    # Set limits and remove axes
    plt.xlim(-1, len(layers) * 1.5)
    plt.ylim(-1, max(input_layer, *hidden_layers, output_layer))
    plt.axis('off')
    plt.title('Neural Network Diagram for Sentiment Analysis on Amazon Book Reviews', fontsize=18)
    plt.show()
