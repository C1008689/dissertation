...
def plot_neural_network_contextual():
    ...
    # Annotations
    plt.text(-0.5, input_layer / 2 - 0.5, 'Input Layer\n(Book Reviews)', fontsize=14, ha='center', va='center', bbox=dict(facecolor='white', alpha=0.5))
    for i in range(1, len(hidden_layers) + 1):
        plt.text(i, hidden_layers[i-1] / 2 - 0.5, f'Hidden Layer {i}\n(Feature Extraction)', fontsize=14, ha='center', va='center', bbox=dict(facecolor='white', alpha=0.5))
    plt.text(len(hidden_layers) + 1, output_layer / 2 - 0.5, 'Output Layer\n(Sentiment Classification)', fontsize=14, ha='center', va='center', bbox=dict(facecolor='white', alpha=0.5))
